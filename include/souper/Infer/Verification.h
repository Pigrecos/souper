...
struct RefinementProblem {
  ...
  inline RefinementProblem ReplacePhi(souper::InstContext &IC, std::map<Block *, size_t> &Change);
...
inline void collectPhis(souper::Inst *I, std::map<souper::Block *, std::set<souper::Inst *>> &Phis);

inline std::unordered_set<RefinementProblem, RefinementProblem::Hash> explodePhis(InstContext &IC, RefinementProblem P);
